...
exe = str(Path(ns.candidate) / "selftest_target.exe")
with open(ns.out, "w", encoding="utf-8", newline="\n") as out:
    for line in open(ns.cases, encoding="utf-8"):
        ...
        try:
            p = subprocess.run([exe, str(c["input"]["ts"]), str(c["input"]["width"])], capture_output=True, text=True, timeout=10)
            if p.returncode == 0:
                obs.update(status="ok", value=int(p.stdout.strip()))
            else:
