...
def _draw(title: str, options: list[tuple[str, str, bool]], at: int, ticked: set[int] | None) -> Text:
    out = Text.assemble((f"  {title}\n", "bold"))
    for i, (label, note, enabled) in enumerate(options):
        here = i == at
        ...
        out.append(f"   {note}\n" if note else "\n", style="dim")
    out.append("  ↑↓ move   " + ("space tick   " if ticked is not None else "") + "enter choose   esc cancel", style="dim")
    return out
...
def _menu(console: Console, title: str, options, start: int, ticked: set[int] | None) -> list[int] | None:
    ...
    with Live(_draw(title, options, at, ticked), console=console, auto_refresh=False, transient=True) as live:
        while True:
            key = _key()
            if key in ("up", "down"):
                ...
                at = int(key) - 1
            live.update(_draw(title, options, at, ticked), refresh=True)
