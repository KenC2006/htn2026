"""python -m ratchet ...   Works from any shell: switches to the project's own Python and loads env/secrets.env itself."""
import os
import subprocess
import sys
...
if py and Path(sys.prefix).resolve() != py.parents[1].resolve():
    sys.exit(subprocess.call([str(py), "-m", "ratchet", *sys.argv[1:]], cwd=os.getcwd()))
